...
void QEditorMainWindow::addProjectFactory(const std::string& class_name, editor::project::FactoryFunction factory) noexcept
{
    _project_factories.emplace(class_name, std::move(factory));
}

auto QEditorMainWindow::getProjectFactory(const std::string& class_name) const noexcept -> const editor::project::FactoryFunction&
{
...
{
    // Find a project file
    QString project_file_path = QFileDialog::getOpenFileName(this, tr("Open project file..."), QEditorApp::instance().settings().get("projects.location").toString(), tr("Project files (*.mprj)"));
    ...
    // Call the slot
    openProject({ project_class.toStdString(), "", QFileInfo{ project_file_path } });
}
...
void QEditorMainWindow::openProject(const editor::project::FactoryData& data) noexcept
{
