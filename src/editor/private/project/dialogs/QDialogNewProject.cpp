...
QDialogNewProject::QDialogNewProject(QEditorMainWindow* main_window)
    ...
    , _model{ main_window->projectFactories() }
{
    _ui->setupUi(this);

    // Setup all default dialog connections
    connect(_ui->projectDialogCancel, &QPushButton::clicked, this, &QDialogNewProject::checkAndFinish);
    connect(_ui->projectDialogCreate, &QPushButton::clicked, this, &QDialogNewProject::checkAndFinish);
    connect(this, &QDialogNewProject::finished, main_window, &QEditorMainWindow::openProject);
    ...
    _ui->projectLocationValue->setText(QEditorApp::instance().settings().get("projects.location").toString());
...
void editor::QDialogNewProject::checkAndFinish() noexcept
{
