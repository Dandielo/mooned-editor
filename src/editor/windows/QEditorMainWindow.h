...
public:
    ...
    auto settings() noexcept -> QSettings& { return _editor_settings; }
    auto settings() const noexcept -> const QSettings& { return _editor_settings; }

    //! \returns The map of all registered project factories.
    auto project_factories() const noexcept -> const auto& { return _project_factories; }

    //! Adds a project factory for the given class name.
    void add_project_factory(const std::string& class_name, editor::project::FactoryFunction factory) noexcept;

    //! \returns The project factory for the requested project class name.
    auto get_project_factory(const std::string& class_name) const noexcept -> const editor::project::FactoryFunction&;
...
public slots:
    ...
    void open_project(const editor::project::FactoryData& data) noexcept;
...
private:
    ...
    //! Holds persistent settings for the whole editor application.
    QPersistentSettings _editor_settings;
